...
const getTranslationBaseUrl = (): string => {
  if (isProduction) {
    return (
      process.env.NEXT_PUBLIC_TRANSLATIONS_BASE_URL ||
      "https://your-azure-storage.blob.core.windows.net/translations"
    );
  }
  return "/locales";
};
...
const loadTranslationFromFile = async (
  ...
  try {
    if (isClient) {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to load ${namespace} translations: ${response.status}`,
        );
      }
...
const loadTranslationFromHttp = async (
  namespace: TranslationNamespace,
): Promise<TranslationData> => {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to load ${namespace} translations from ${url}: ${response.status}`,
      );
    }
...
export const getTranslationsForPages = async (
...
export const getTranslationsForApp = async (
  namespaces: TranslationNamespace[] = ["common"],
) => {
  const translations = await loadTranslations(namespaces);
  return translations;
